...
class ScalarVariable:
    ...
    def getCPPString(self):
        ...
        if self.attributes["type"] == "double":
            ...
            print(
                "WARNING. UNRECOGNIZED TYPE: {0}. Parsing as: \n{1}".format(
                    self.attributes["type"], rs
                )
            )
        return rs
...
def providesJacobian(filename):
    doc = ET.parse(filename)
    providesDirDerivative = doc.findall(
        "ModelExchange/[@providesDirectionalDerivative='true']"
    )
...
def compile_str(legend, variables, using_str, provides_jacobian):
    ...
    rs += "class {0}:\n\tpublic adevs::FMI<{1}>\n{{\n\tpublic:\n\t\t{0}():\n".format(
        legend["modelName"], legend["convType"]
    )  # Format first part of class
    rs += '\t\t\tadevs::FMI<{1}>\n\t\t\t(\n\t\t\t\t"{0}",\n\t\t\t\t"{2}",\n\t\t\t\t"{3}",\n\t\t\t\t{4},\n\t\t\t\t{5},\n\t\t\t\t"{6}",\n\t\t\t\t1E-8,0,0.0,\n\t\t\t\t{7}\n\t\t\t)\n\n\t\t{{\n\t\t}}\n'.format(
        legend["modelName"],
        legend["convType"],
        legend["guid"],
        legend["resourceLocation"],
        legend["derNum"],
        legend["indicatorNum"],
        legend["sharedLocation"],
        jacobian_init_str,
    )  # Format FMI constructor call
...
def print_help():
    print(
        "This program converts .xml files to .h files under the FMI standard found at www.fmi-standard.org"
    )
    print(
        "Example: 'xml2cpp -x target_xml -type type -f shared_object_file -r resource_location -o output_name'"
    )
...
if __name__ == "__main__":
    ...
    legend["sharedLocation"] = (
        file_loc  # This is the location of the shared object file produced by omc
    )
    legend["resourceLocation"] = (
        res_loc  # This is the location of json files produced by omc
    )
    ...
    write_file(
        output_file,
        compile_str(legend, variables, USING_STRING, providesJacobian(filename)),
    )
