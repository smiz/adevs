...
class LookLeft : public adevs::Atomic<adevs::CellEvent<int>> {
  public:
    ...
    void delta_ext(double e, std::list<adevs::CellEvent<int>> const &xb) {
        c += e;
        l = (*(xb.begin())).value;
    }
    void delta_conf(std::list<adevs::CellEvent<int>> const &xb) {
        delta_int();
        delta_ext(0.0, xb);
    }
    double ta() { return P - c; }
    // Output function
    void output_func(std::list<adevs::CellEvent<int>> &yb) {
