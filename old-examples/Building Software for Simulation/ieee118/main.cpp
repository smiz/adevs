...
int main(int argc, char const** argv) {
    ...
    ElectricalModel* dynamics =
        new ElectricalModel(new ElectricalModelEqns(data));
    Digraph<BasicEvent*>* model = new Digraph<BasicEvent*>();
    model->add(control);
    model->add(genr_fail);
    model->add(dynamics);
    model->couple(genr_fail, genr_fail->genr_fail, dynamics,
                  ElectricalModelEqns::GenrTrip);
    model->couple(control, control->sample_setting, dynamics,
                  ElectricalModelEqns::SetGenrSample);
    for (unsigned i = 0; i < data->getNodeCount(); i++) {
        ...
        model->couple(aggr_net, aggr_net->load, dynamics,
                      ElectricalModelEqns::LoadAdj);
    }
    // Simulate it
