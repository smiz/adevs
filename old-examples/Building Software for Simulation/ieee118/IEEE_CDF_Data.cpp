...
IEEE_CDF_Data::IEEE_CDF_Data(char const* data_file) {
    ...
    do {
        ...
        if (flag != -999) {
            bus_data.ID = flag - 1;
            read_field(28, 33);
            bus_data.v = atof(buffer);
            read_field(34, 40);
            bus_data.theta = atof(buffer);
            bus_data.theta *= DEGS_TO_RADS;
            read_field(41, 49);
            bus_data.load_mw = atof(buffer) / Sbase;
            read_field(50, 59);
            bus_data.load_mvar = atof(buffer) / Sbase;
            read_field(60, 67);
            bus_data.genr_mw = atof(buffer) / Sbase;
            read_field(68, 75);
            bus_data.genr_mvar = atof(buffer) / Sbase;
            read_field(107, 114);
            bus_data.G = atof(buffer);
            read_field(115, 122);
            bus_data.B = atof(buffer);
...
void IEEE_CDF_Data::setInitialConditions() {
    ...
    Complex* I =
        new Complex[getNodeCount()];  // For power injected by the loads
    ...
    for (unsigned i = 0; i < genr_nodes.size(); i++) {
        ...
        I[node_id] -=
            V[i] *
            conj(E / g.Xd);  // Subtract the power injected by the generator
    }
    // Cleanup
    delete[] I;
    delete[] V;
