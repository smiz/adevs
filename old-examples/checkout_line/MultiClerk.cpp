...
MultiClerk::MultiClerk() : Digraph<shared_ptr<Customer>>() {
    // Create and add component models
    shared_ptr<Decision> decision = make_shared<Decision>();
    add(decision);
    shared_ptr<Clerk> clerks[NUM_LINES];
    for (int ii = 0; ii < NUM_LINES; ii++) {
        clerks[ii] = make_shared<Clerk>();
        add(clerks[ii]);
    }
