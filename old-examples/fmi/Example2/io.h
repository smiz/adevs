...
class CommandSig : public SimObject {
  public:
    CommandSig(double T1, double T2) : SimObject(), T1(T1), T2(T2) {}
    double getT1() const { return T1; }
    double getT2() const { return T2; }
    SimObject* clone() const { return new CommandSig(T1, T2); }

  private:
    double const T1, T2;
};

class SampleSig : public SimObject {
  public:
    SampleSig(double q1, double q2) : SimObject(), q1(q1), q2(q2) {}
    double getQ1() const { return q1; }
    double getQ2() const { return q2; }
    SimObject* clone() const { return new SampleSig(q1, q2); }
