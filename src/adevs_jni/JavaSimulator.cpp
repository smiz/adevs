...
static void clean_up_orphaned_models(JavaSimulator* sim) {
    if (!sim->structure_change_occurred) {
        return;
    }
 ...
 */
JNIEXPORT jlong JNICALL Java_adevs_Simulator_createCppSimulator(JNIEnv* env,
                                                                jobject caller,
                                                                jobject model) {
    jclass clazz;
    ...
    sim->jdevs_model_transition =
        env->GetMethodID(clazz, "model_transition", "()Z");
    ...
    sim->jatomic_dext =
        env->GetMethodID(clazz, "delta_ext", "(DLjava/util/Collection;)V");
    sim->jatomic_dcon =
        env->GetMethodID(clazz, "delta_conf", "(Ljava/util/Collection;)V");
    sim->jatomic_ta = env->GetMethodID(clazz, "ta", "()D");
    sim->jatomic_out =
        env->GetMethodID(clazz, "output_func", "(Ljava/util/Collection;)V");
    env->DeleteLocalRef(clazz);
    ...
    sim->jiterator_next =
        env->GetMethodID(clazz, "next", "()Ljava/lang/Object;");
    ...
    jfieldID shared_jcollection =
        env->GetFieldID(clazz, "shared_coll", "Ljava/util/Collection;");
    jfieldID shared_jevent =
        env->GetFieldID(clazz, "shared_event", "Ladevs/Event;");
    env->DeleteLocalRef(clazz);
    // adevs.Event
    ...
    sim->jevent_listener_out =
        env->GetMethodID(clazz, "outputEvent", "(Ladevs/Event;D)V");
    sim->jevent_listener_delta =
        env->GetMethodID(clazz, "stateChange", "(Ladevs/Atomic;D)V");
    env->DeleteLocalRef(clazz);
    // adevs.Network
    clazz = env->FindClass("adevs/Network");
    sim->jnetwork_class = env->NewGlobalRef(clazz);
    sim->jnetwork_route = env->GetMethodID(
        clazz, "route",
        "(Ljava/lang/Object;Ladevs/Devs;Ljava/util/Collection;)V");
    sim->jnetwork_get_components =
        env->GetMethodID(clazz, "getComponents", "(Ljava/util/Collection;)V");
    env->DeleteLocalRef(clazz);
    // Done!
 ...
 * Signature: (J)D
 */
JNIEXPORT jdouble JNICALL Java_adevs_Simulator_nextEventTime(JNIEnv* env,
                                                             jobject,
                                                             jlong peer_id) {
    if (peer_id == 0) {
        return DBL_MAX;
 ...
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_adevs_Simulator_execNextEvent(JNIEnv* env, jobject,
                                                          jlong peer_id) {
    if (peer_id == 0) {
        return;
    }
 ...
 * Signature: (DJ)V
 */
JNIEXPORT void JNICALL Java_adevs_Simulator_execUntil(JNIEnv* env, jobject,
                                                      jdouble tend,
                                                      jlong peer_id) {
    if (peer_id == 0) {
        return;
 ...
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_adevs_Simulator_computeNextOutput(JNIEnv* env,
                                                              jobject,
                                                              jlong peer_id) {
    if (peer_id == 0) {
        return;
...
JNIEXPORT void JNICALL Java_adevs_Simulator_computeNextState(
    ...
    jobject iter = sim->jenv->NewGlobalRef(
        sim->jenv->CallObjectMethod(collection, sim->jcollection_iterator));
    while (sim->jenv->CallBooleanMethod(iter, sim->jiterator_hasNext) ==
           JNI_TRUE) {
        sim->jenv->PushLocalFrame(16);
        jobject event = sim->jenv->CallObjectMethod(iter, sim->jiterator_next);
        // Get the cpp peer of the model
        jobject java_model =
            sim->jenv->GetObjectField(event, sim->jevent_model);
        Devs<java_io>* adevs_model = (Devs<java_io>*)(sim->jenv->GetLongField(
            java_model, sim->jdevs_cpp_peer));
        // Get the cpp value
        jobject java_value =
            sim->jenv->GetObjectField(event, sim->jevent_value);
        // Create an adevs event and put it into the input bag
        sim->input_bag.insert(
            Event<java_io>(adevs_model, sim->jenv->NewGlobalRef(java_value)));
 ...
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_adevs_Simulator_destroyCppSimulator(JNIEnv* env,
                                                                jobject,
                                                                jlong peer_id) {
    if (peer_id == 0) {
        return;
