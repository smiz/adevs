...
void JavaNetwork::getComponents(Set<Devs<java_io>*>& c)
{
	...
	owner.jenv->CallVoidMethod(getJavaObjRef(),owner.jnetwork_get_components,owner.jsimulator_shared_collection);
...
void JavaNetwork::route(const java_io& x, Devs<java_io>* model, Bag<Event<java_io> >& r)
{
	...
	owner.jenv->CallVoidMethod(getJavaObjRef(),owner.jnetwork_route,
...
{
	bool result = (owner.jenv->CallBooleanMethod(getJavaObjRef(),owner.jdevs_model_transition) == JNI_TRUE);
	return result;
}
