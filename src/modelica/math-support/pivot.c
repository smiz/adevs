...
#define swap(a,b) { long _swap=a; a=b; b=_swap; }
...
*/
long maxsearch( double *A, long start, long n_rows, long n_cols, long *rowInd, long *colInd, long *maxrow, long *maxcol, double *maxabsval)
{
  /* temporary variables */
  long row;
  long col;
...
*/
long pivot( double *A, long n_rows, long n_cols, long *rowInd, long *colInd )
{
  /* parameter, determines how much larger an element should be before rows and columns are longerchanged */
  ...
  for(row=0; row<min(n_rows,n_cols); row++)
  {
    ...
    /* get pivot (without abs, may have changed because of row/column longerchange */
    pivot = get_pivot_matrix_elt(A,row,row);
    /* longernal error, pivot element should never be zero if maxsearch succeeded */
    assert(pivot != 0);
