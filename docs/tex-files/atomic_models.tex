...
Atomic models are the basic building blocks of a DEVS model. The behavior of an atomic model is described by its state transition functions (internal, external, and confluent), its output function, and its time advance function. Within \adevs, these aspects of an atomic model are implemented by sub-classing the \classname{Atomic} class and implementing its pure virtual methods that correspond to the internal, external, and confluent transition functions, the output function, and the time advance function.
...
The \classname{Clerk} described in Section \ref{chapter:intro} demonstrates all of the aspects of an \classname{Atomic} model. We'll use it to demonstrate how an \classname{Atomic} model generates output, processes input, and schedules internal events. Here is the \classname{Clerk}'s class definition:
\begin{verbatim}
include "adevs.h"
#include "Customer.h"
#include <list>
...
This new clerk has two state variables. The first records the time remaining before the clerk is willing to interrupt the processing of a customer. The second is the list of customers waiting to be served. Here is the header file for the new clerk model, which we will call \classname{Clerk2}.
\begin{verbatim}
#include "adevs.h"
#include "Customer.h"
#include <list>
...
The \classname{Clerk2} constructor sets the clerk's clock and interruption timers to zero.
\begin{verbatim}
Clerk2::Clerk2():
Atomic<IO_Type>(),
preempt(0.0),
...
\end{verbatim}
The output function of the \classname{Clerk2} model sets the exit time of the departing customer and then ejects that customer via the ``depart" port.
\begin{verbatim}
...
The \classname{Clerk2}'s external transition function works as follows. When a new customer arrives, the clerk first advanced its clock by the elapsed time. Next, she reduces the time remaining to process the current customer. This reduction reflects the amount of time that has already been spent on the customer's order, which is the time elapsed since the clerk's last change of state. Then the clerk decrements the time remaining before she is willing to interrupt the processing of a large order; this timer is also decremented by the elapsed time.
...
The last function to implement is the confluent transition function. The \classname{Clerk2} model has the same confluent transition as the \classname{Clerk} that is described in section \ref{chapter:intro}:
\begin{verbatim}
...
The evolution of the \classname{Clerk2} line is depicted in Fig. \ref{fig:clerk2_line}. To time 11 the line evolves just as it did with the \classname{Clerk} model. At time 11 the \classname{Clerk2} changes the course of the simulation by moving a customer with a small order to the front of the line.
\begin{figure}[ht]
\centering
