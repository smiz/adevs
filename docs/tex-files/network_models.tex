...
The \methodname{add} method does three things. First, it checks that the network is not being added to itself. This is illegal and would cause the simulator to throw an exception. Next, it adds the new component to its set of components. Last, the \classname{SimpleDigraph} makes itself the component's parent. The last step is needed so that the simulator can climb up and down the model tree. If this step is omitted then the recursive routing of events will fail. Here is the implementation of the \methodname{add} method.
\begin{verbatim}
