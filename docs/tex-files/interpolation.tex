...
The \classname{InterPoly} class approximates smooth continuous signal by fitting a polynomial to the available data points. The approximating polynomial is guaranteed to pass through every available data point, and in many cases it provides a reasonable approximation to the original signal between the available data points. The most familiar example of an interpolating polynomial is a line that connects two data points $(t_1,x_1)$ and $(t_2,x_2)$; the connecting line is
\begin{equation*}
...
\end{verbatim}
where u is an array of data values, dt is the time spacing of the data points, n is the number of data points, and t0 is the time instant of the first data point (i.e., the data point $i$ is at time $t_0+i \cdot dt$). Both constructors make copies of the supplied arrays, and changes to the array values will not be reflected by the \classname{InterPoly} object. The third way is to assign new data point values to an existing polynomial by calling the \classname{InterPoly} method
\begin{verbatim}
void setData(const double* u, const double* t = NULL)
\end{verbatim}
where u is the new set of data values and t is (optionally) the new set of time points. This method requires that the number of data points in u (and t if used) be equal to the number of points supplied to the \classname{InterPoly} constructor.

There are three methods for computing interpolated values: the \methodname{interpolate} method, the overloaded \methodname{operator()}, and the \methodname{derivative} method. The method signatures are listed below:
\begin{verbatim}
double interpolate(double t) const
double operator()(double t) const
double derivative(double t) const
\end{verbatim}
The \methodname{interpolate} method and \methodname{operator()} give the value of the interpolating polynomial at the time point t. The \methodname{derivative} method gives the value of the first time derivative of the interpolating polynomial as an approximation of the first time derivative of the original function. For example, if the data describes the position of an object through time then the \methodname{derivative} method gives and approximation of the object's velocity.
