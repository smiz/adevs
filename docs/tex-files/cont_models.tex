...
I will use the OpenModelica compiler to demostrate how this is converted to an FMI compliant object file that can be used with adevs. Assume that the compiler \verbatim{omc} is in our path. We begin by creating a Modelica script file called \verbatim{makefmi.mos} and put into this file the following commands.
\begin{verbatim}
loadFile("CherryBomb.mo");
translateModelFMU(CherryBomb,"2.0");
\end{verbatim}
The first line loads the CherryBomb model that is listed above. The second line generates and FMI object from this model that is compliant with the FMI v2 standard. This FMI object will be packed with several other files in a zipped file called \verbatim{CherryBomb.fmu}. The OpenModelica compiler will also produce a bunch of other junk that you don't need, and these extra files can be deleted with the following set of command.
\begin{verbatim}
rm -f CherryBomb_*
rm -f CherryBomb.c
\end{verbatim}
...
\end{verbatim}
This will create directory called \verbatim{sources}, which you can delete, a file called \verbatim{modelDescription.xml}, which you will need, and a file \verbatim{CherryBomb.so}, which you will need. This shared object file will be in the directory called \verbatim{binaries/linux64}.

The file \verbatim{modeDescription.xml} contains all of the information you will need to program your simulator to access the variables of the Modelica model. The listing of the description file for the cherry bomb is shown below. The significance of the data in this file will become apparent as we construct the rest of the simulation software that will interact with this model.
\begin{verbatim}
...
\begin{enumerate}
\item The model name that appears in the \verbatim{modelDecription.xml} within the tag \verbatim{modelName}.
\item The GUID string for the FMI object that appears within the tag \verbatim{guid} just beneath the \verbatim{modelName} in \verbatim{modelDescription.xml}.
\item The number of continuous state variables in the Modelica model, which is determined by counting the number of variable descriptions containing the tagged \verbatim{Real derivative}. There are three of these for the CherryBomb model.
\item The number of state event conditions in the model, which is contained in the tag \verbatim{numberOfEventIndicators}. 
