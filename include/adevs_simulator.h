 ...
 * </p>
 *
 * The OptSimulator class makes the callbacks just before its execUntil()
 ...
 * for Network models; only for Atomic models.
 */
...
void Simulator<X>::unschedule_model(Devs<X>* model)
{
	if (model->typeIsAtomic() != NULL)
	{
		...
		if (sched.getMinimum() == model) sched.removeMinimum();
