...
class TestCircuitModelicaExt:
	public TestCircuitModelica,
	public CircuitOutput
{
	public:
		TestCircuitModelicaExt():
			TestCircuitModelica(),
			CircuitOutput(){}
	...
	public Atomic<IOType>
{
	public:
		...
		void delta_ext(double e, const IOBag& xb)
		{
			// Duty cycle is applied at start of each period
			h -= e;
			duty = xb[0].value;
		}
		void delta_conf(const IOBag& xb)
		{
			pos = next_position();
...
class TestCircuitAdevsExt:
	public TestCircuitAdevs,
	public CircuitOutput
{
	public:
		TestCircuitAdevsExt():
		...
		void external_event(double* q, double e, const IOBag& xb)
		{
			TestCircuitAdevs::external_event(q,e,xb);
			set_switches(xb);
			TestCircuitAdevs::external_event(q,0.0,xb);
		}
		void confluent_event(double* q, const bool* state_event, const IOBag& xb)
		{
			TestCircuitAdevs::confluent_event(q,state_event,xb);
			set_switches(xb);
			TestCircuitAdevs::confluent_event(q,state_event,xb);
		}
		void output_func(const double* q, const bool* state_event, IOBag& yb)
		{
			IOType y;
			double duty_cycle;
	...
	private:
		...
		void set_switches(const IOBag& xb)
		{
			for (auto x: xb)
			{
				switch(x.port)
