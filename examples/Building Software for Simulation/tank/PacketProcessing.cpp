...
void PacketProcessing::delta_ext(double e, Bag<SimEvent> const &xb) {
    ...
    for (Bag<SimEvent>::const_iterator iter = xb.begin(); iter != xb.end();
         iter++) {
...
void PacketProcessing::delta_conf(Bag<SimEvent> const &xb) {
    delta_int();
    delta_ext(0.0, xb);
}
