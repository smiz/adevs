...
class Sq : public adevs::Atomic<int> {
  public:
    ...
    void delta_int() {
        q.pop_front();
        ttg = r.exponential(1.0) + 1.0;
    }
    void delta_ext(double e, adevs::Bag<int> const &xb) {
        if (!q.empty()) {
            ttg -= e;
        }
        for (adevs::Bag<int>::const_iterator i = xb.begin(); i != xb.end();
             i++) {
            q.push_back(*i);
        }
    }
...
class Ql : public adevs::Network<int> {
  public:
    Ql(int size) : adevs::Network<int>(), size(size), sq(new Sq[size]) {
    ...
    void route(int const &value, adevs::Devs<int>* model,
        ...
        if (model == this) {
            r.insert(adevs::Event<int>(&(sq[0]), value));
        }
