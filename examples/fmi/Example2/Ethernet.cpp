...
const int Rx::from_media = 0;
const int Rx::to_app = 1;
...
void Rx::delta_ext(double e, const Bag<IO_Type>& xb)
{
	Bag<IO_Type>::const_iterator iter;
...
void Rx::delta_conf(const Bag<IO_Type>& xb)
{
	delta_int();
	delta_ext(0.0,xb);
}

void Rx::output_func(Bag<IO_Type>& yb)
{
	IO_Type msg;
	msg.port = to_app;
	msg.value = data;
...
const int Tx::from_app = 0;
const int Tx::to_media = 1;
const int Tx::from_media = 2;
...
void Tx::delta_ext(double e, const Bag<IO_Type>& xb)
{
	...
	if (startMsgs > 0) busy = true;
	else if (cmpltMsgs > 0) busy = false;
...
void Tx::delta_conf(const Bag<IO_Type>& xb)
{
	delta_int();
	delta_ext(0.0,xb);
}

void Tx::output_func(Bag<IO_Type>& yb)
{
	IO_Type msg;
	msg.port = to_media;
...
const int NetworkCard::to_media = 1;
const int NetworkCard::from_media = 2;
const int NetworkCard::to_app = 3;
const int NetworkCard::from_app = 4;

NetworkCard::NetworkCard(int addr, int maxTries):
	Digraph<SimObject*>()
{
	Rx* rx = new Rx(addr);
	tx = new Tx(maxTries);
