...
const double x0[2] = { 0.0, sqrt(2.0) };
	...
	public Atomic<event_t>
{
	public:
		...
		void delta_conf(const Bag<event_t>& xb)
		{
			xj = xb[0].x;
			xi = xl = xn;
			pick_next(q);
		}
		void delta_ext(double e, const Bag<event_t>& xb)
		{
		...
		void output_func(Bag<event_t>& y)
		{
			event_t yy;
			yy.k = i;
			yy.x = xn;
	...
	public Atomic<event_t>
{
	public:
			...
			t(0.0)
		{
			...
			x[1] = x0[1];
			err[0] = err[1] = 0.0;
			cout << t << " " << x[0] << " " << x[1] << " 0 0 " << x[0] << " " << x[1] << endl;
		}
		void delta_int() { t += ta(); }
		void delta_ext(double e, const Bag<event_t>& xb)
		{
			...
			cout << t << " " << x[0] << " " << x[1] << " " << err[0] << " " << err[1] << " " << x1 << " " << x2 << endl;
		}
		void delta_conf(const Bag<event_t>& xb)
		{
			delta_ext(ta(),xb);
