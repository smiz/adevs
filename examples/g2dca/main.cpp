...
void drawSpace()
{
	...
	glRasterPos2f(20,20);
	glColor3f(1.0,1.0,1.0);
	glutBitmapString(GLUT_BITMAP_HELVETICA_12,reinterpret_cast<const unsigned char*>(name.c_str()));
	glutSwapBuffers();
}
...
void simulateSpace()
{
	...
	if (cell_space == NULL)
	{
		...
		for (int x = 0; x < WIDTH; x++)
		{
			...
			{
				cell_space->add(
					new Cell(x,y,phase[x][y],&(phase[x][y])),
					x,y);
			}
		}
