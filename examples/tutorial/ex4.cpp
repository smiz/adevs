...
class InstructionSource : public MealyAtomic {
  public:
    ...
    void map_ready_to_core(pin_t ready_pin, pin_t output_pin) {
  ...
  private:
    ...
    /// core
    std::map<pin_t, pin_t> ready_to_output;
...
class Stage : public Atomic {
  public:
    ...
    /// to be passive.
    Stage(int stage)
        : Atomic(), stage(stage), time_to_finish(0), instruction(-1), downstream_ready(true) {}
...
int main() {
    ...
    std::cout << "Rate:         " << ((double)computer->get_instructions_complete()) / (double)time
              << std::endl;
    /// Done!
    return 0;
}
