...
using CVODE = adevs::CVODE<double>;
 ...
 */
class bouncing_ball : public CVODE {
  public:
    /** 
     ...
     */
    bouncing_ball(double h_max) : CVODE(), phase(FALL), sample(false), t(0.0), h_max(h_max) {
        int retval;
     ...
     */
    void cvode_delta_ext(double t, std::list<PinValue> const &) {
        this->t = t;
     ...
     * Schedule an output immediately in response to an input.
     */
    void cvode_delta_conf(std::list<PinValue> const &) {
        sample = true;  // tell cvode_integrate() to take a step size of zero
    }
     ...
     */
    void cvode_output_func(std::list<PinValue> &yb) {
        PinValue event(sample_pin, NV_Ith_S(y, 0));
        yb.push_back(event);
    }
...
class SolutionChecker : public EventListener {
  public:
    SolutionChecker(bouncing_ball* ball, bool to_cout)
        : EventListener(), ball(ball), to_cout(to_cout) {}
